...
{
    public partial class IncomeGroupRestrictionSettingGlobal : BasePage
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            set.IsGlobal = rbnPurchaseGlobal.Checked;
            set.CompanyId = CurrentCompany.CompanyId;
            ...
            if (radQujian.Checked && rbnPurchaseGlobal.Checked)
            {
            ...
            set.IncomeGroupId = rbnPurchaseGlobal.Checked ? (Guid?)null : Request.QueryString["id"] == null ? (Guid?)null : Guid.Parse(Request.QueryString["id"]);

            try
            {
                if (hidIsFirst.Value == "1")
                {
                    IncomeGroupService.InsertIncomeGroupDeductSetting(set, CurrentUser.UserName);
                }
                else
                {
                    if (rbnPurchaseGlobal.Checked)
                    {
                        set.Id = Guid.Parse(hidVId.Value);
                        IncomeGroupService.UpdateIncomeGroupDeductSetting(set, CurrentUser.UserName);
                    }
                    else
                    {
                        IncomeGroupService.UpdateIsGlobal(CurrentCompany.CompanyId, rbnPurchaseGlobal.Checked);
                    }
                }
            }
            catch (Exception ex)
            ...
            RegisterScript("alert('设置收益信息成功！');window.location.href='IncomeGroupRestrictionSettingGlobal.aspx';", true);
