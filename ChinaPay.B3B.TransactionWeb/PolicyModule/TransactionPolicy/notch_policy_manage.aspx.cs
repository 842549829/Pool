...
{
    public partial class notch_policy_manage : BasePage
    {
        ...
        void QueryPolicy(Pagination pagination)
        {
            try
            {
                var querylist = PolicyManageService.QueryNotchPolicies(GetCondition(), pagination);
        ...
        private void QueryNotchPolicy(int pageindex)
        {
        ...
        {
            Response.Redirect("./notch_policy_publish.html");
        }
        ...
        protected void grv_normal_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                PolicyManageService.Audit(PolicyType.Notch, this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                QueryNotchPolicy(pager.CurrentPageIndex);
            }
            //取消审核
            if (e.CommandName == "UnAudited")
            {
                PolicyManageService.CancelAudit(PolicyType.Notch, this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                QueryNotchPolicy(pager.CurrentPageIndex);
            }
            //删除政策
            if (e.CommandName == "del")
            {
                try
                {
                    PolicyManageService.DeleteNotchPolicy(this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                    QueryNotchPolicy(pager.CurrentPageIndex);
                }
                catch (Exception ex)
                {
        ...
        protected void btnAudited_Click(object sender, EventArgs e)
        {
            if (GetCheckBoxValue())
            {
                ...
                PolicyManageService.Audit(PolicyType.Notch, this.CurrentUser.UserName, list_ids.ToArray());
                QueryNotchPolicy(pager.CurrentPageIndex);
                ClaerHidValue();
            }
        }
        ...
        protected void btnUnAudited_Click(object sender, EventArgs e)
        {
            ...
            {
                var list_ids = hidIsAll.Value == "0" ? hidIds.Value.Split(',').Select(Guid.Parse) : PolicyManageService.QueryPolicyIds(GetCondition()).Select(item => item.Key);
                PolicyManageService.CancelAudit(PolicyType.Notch, this.CurrentUser.UserName, list_ids.ToArray());
                QueryNotchPolicy(pager.CurrentPageIndex);
                ClaerHidValue();
            }
        }
        ...
        protected void btnDel_Click(object sender, EventArgs e)
        {
            if (GetCheckBoxValue())
            {
                ...
                try
                {
                    PolicyManageService.DeleteNotchPolicy(this.CurrentUser.UserName, list_ids.ToArray());
                    QueryNotchPolicy(pager.CurrentPageIndex);
                    ClaerHidValue();
                }
                catch (Exception ex)
