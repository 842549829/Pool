<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="base_policy_manage.aspx.cs"
    Inherits="ChinaPay.B3B.TransactionWeb.PolicyModule.TransactionPolicy.base_policy_manage" %>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="btns" colspan="5" style="padding: 0px;">
                            ...
                            <input type="button" class="btn class2" value="批量导入" onclick="javascript:window.location.href='base_policy_import.aspx'" />
        ...
        <asp:GridView ID="grv_normal" runat="server" CssClass="nfo-table info" Width="100%"
            ...
            <Columns>
                <asp:TemplateField>
                    ...
                    <ItemTemplate>
                        <input type="checkbox" class="chkOne" runat="server" value='<%#Eval("id") %>' />
                    </ItemTemplate>
                ...
                <asp:TemplateField HeaderText="操作">
                    <ItemTemplate>
                        <asp:LinkButton CommandArgument='<%#Eval("id") %>' CommandName='<%#Eval("SuditName") %>'
                        ...
                        <a href="javascript:ModifyCommissionBase('<%#Eval("id") %>','<%#Eval("InternalCommission") %>','<%#Eval("SubordinateCommission") %>','<%#Eval("ProfessionCommission") %>','<%#Eval("IsInternal") %>','<%#Eval("IsPeer") %>');">
                            修改返佣</a><br />
                        <asp:LinkButton ID="LinkButton1" CommandArgument='<%#Eval("id") %>' CssClass="del_click"
                            CommandName='del' runat="server">删 除</asp:LinkButton><br />
                        <a href="base_policy_edit.aspx?Id=<%#Eval("id") %>&Type=Update">修改详细</a><br /> <a href="base_policy_edit.aspx?Id=<%#Eval("id") %>&Type=Copy">
                            政策复制</a>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
