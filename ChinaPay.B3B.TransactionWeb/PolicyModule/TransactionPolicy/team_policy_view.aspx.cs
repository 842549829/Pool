...
{
    public partial class team_policy_view : BasePage
    {
        ...
        protected void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            QueryTeamPolicyView(1);
        }

        private void QueryTeamPolicyView(int pageindex)
        {
        ...
        void QueryPolicy(Pagination pagination)
        {
            try
            {
                ...
                var list_query = ChinaPay.B3B.Service.PolicyMatch.PolicyMatchServcie.GetTeamPolicies(this.CurrentCompany.CompanyId, paramer, item => item.Rebate, OrderMode.Descending);
                var list = from item in list_query
                           let item_normal = item.OriginalPolicy as TeamPolicyInfo
                           select new
                           {
                               //政策编号
                               id = item_normal.Id,
                               ...
                               (item_normal.IsInternal ? "内部: " + (item_normal.InternalCommission * 100).TrimInvaidZero() + "%<br />" : "")
                                             + "下级: " + (item_normal.SubordinateCommission * 100).TrimInvaidZero() + "%<br />"
                               ...
                               ////回程日期
                               //ReturnDates = item_normal.VoyageType == VoyageType.OneWay ? "" : (item_normal.ReturnDateStart.Value.ToString("yyyy-MM-dd") + "<br />" + item_normal.ReturnDateEnd.Value.ToString("yyyy-MM-dd")),
                               //操作人
                               ...
                               Policy_link = item_normal.Owner == this.CurrentCompany.CompanyId ? "<a href='team_policy_edit.aspx?Id=" + item_normal.Id + "&Type=Update&Check=view'>修改详细</a>" : ""
                           };
