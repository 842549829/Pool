...
namespace ChinaPay.B3B.TransactionWeb.PolicyModule.TransactionPolicy
{
    public partial class base_policy_manage : BasePage
    {
        ...
        void QueryPolicy(Pagination pagination)
        {
            try
            {
                var querylist = PolicyManageService.QueryNormalPolicies(GetCondition(), pagination);
        ...
        {
            Response.Redirect("./base_policy_publish.htm");
        }
        ...
        protected void grv_normal_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                PolicyManageService.Audit(PolicyType.Normal, this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                QueryNormalPolicy(pager.CurrentPageIndex);
            }
            //取消审核
            if (e.CommandName == "UnAudited")
            {
                PolicyManageService.CancelAudit(PolicyType.Normal, this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                QueryNormalPolicy(pager.CurrentPageIndex);
            }
            //删除政策
            if (e.CommandName == "del")
            {
                try
                {
                    PolicyManageService.DeleteNormalPolicy(this.CurrentUser.UserName, Guid.Parse(e.CommandArgument.ToString()));
                    QueryNormalPolicy(pager.CurrentPageIndex);
                }
                catch (Exception ex)
                {
        ...
        protected void btnAudited_Click(object sender, EventArgs e)
        {
            if (GetCheckBoxValue())
            {
                ...
                PolicyManageService.Audit(PolicyType.Normal, this.CurrentUser.UserName, list_ids.ToArray());
                QueryNormalPolicy(pager.CurrentPageIndex);
                ClaerHidValue();
            }
        }
        ...
        protected void btnUnAudited_Click(object sender, EventArgs e)
        {
            ...
            {
                var list_ids = hidIsAll.Value == "0" ? hidIds.Value.Split(',').Select(Guid.Parse) : PolicyManageService.QueryPolicyIds(GetCondition()).Select(item => item.Key);
                PolicyManageService.CancelAudit(PolicyType.Normal, this.CurrentUser.UserName, list_ids.ToArray());
                QueryNormalPolicy(pager.CurrentPageIndex);
                ClaerHidValue();
            }
        }
        ...
        protected void btnDel_Click(object sender, EventArgs e)
        {
            if (GetCheckBoxValue())
            {
                ...
                try
                {
                    PolicyManageService.DeleteNormalPolicy(this.CurrentUser.UserName, list_ids.ToArray());
                    QueryNormalPolicy(pager.CurrentPageIndex);
                    ClaerHidValue();
                }
                catch (Exception ex)
