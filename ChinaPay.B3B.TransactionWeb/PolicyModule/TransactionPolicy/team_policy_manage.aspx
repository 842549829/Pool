<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="team_policy_manage.aspx.cs" Inherits="ChinaPay.B3B.TransactionWeb.PolicyModule.TransactionPolicy.team_policy_manage" %>
                ...
                <asp:TemplateField HeaderText="操作">
                    <ItemTemplate>
                        ...
                        <a href="javascript:ModifyCommissionBase('<%#Eval("id") %>','<%#Eval("InternalCommission") %>','<%#Eval("SubordinateCommission") %>','<%#Eval("ProfessionCommission") %>','<%#Eval("IsInternal") %>','<%#Eval("IsPeer") %>');">
                            修改返佣</a><br />
                        <asp:LinkButton ID="LinkButton2" CommandArgument='<%#Eval("id") %>' CssClass="del_click" CommandName='del'
                            runat="server">删 除</asp:LinkButton> <br />
                        <a href="team_policy_edit.aspx?Id=<%#Eval("id") %>&Type=Update">修改详细</a> <br />
                        <a href="team_policy_edit.aspx?Id=<%#Eval("id") %>&Type=Copy">政策复制</a>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        </asp:GridView>
