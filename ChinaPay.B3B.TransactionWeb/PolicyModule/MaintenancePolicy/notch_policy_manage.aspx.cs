...
namespace ChinaPay.B3B.TransactionWeb.PolicyModule.MaintenancePolicy
{
    public partial class notch_policy_manage : BasePage
    {
        ...
        void QueryPolicy(Pagination pagination)
        {
            try
            {
                var listQuery = PolicyManageService.QueryNotchPolicies(GetCondition(), pagination);
        ...
        protected void btnSavelock_Click(object sender, EventArgs e)
        {
            if (CheckBoxValue())
            {
                ...
                try
                {
                    PolicyManageService.LockPolicy(PolicyType.Notch, this.CurrentUser.UserName, this.txtlockReason.Text, CurrentCompany.CompanyType == CompanyType.Platform ? OperatorRole.Platform : OperatorRole.User, listIds.ToArray());
                    QueryNotchPolicy(grv_normal.PageIndex + 1); ClaerHidValue();
                }
                catch (Exception ex)
                {
