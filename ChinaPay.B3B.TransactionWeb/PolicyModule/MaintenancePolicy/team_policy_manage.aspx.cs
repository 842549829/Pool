    ...
    public partial class team_policy_manage : BasePage
    {
        ...
        protected void btnSavelock_Click(object sender, EventArgs e)
        {
            if (CheckBoxValue())
            {
                ...
                try
                {
                    PolicyManageService.LockPolicy(PolicyType.Team, this.CurrentUser.UserName, this.txtlockReason.Text, CurrentCompany.CompanyType == CompanyType.Platform ? OperatorRole.Platform : OperatorRole.User, listIds.ToArray());
                    QueryTeamPolicy(grv_normal.PageIndex + 1);
                }
                catch (Exception ex)
                {
        ...
        protected void btnSaveunlock_Click(object sender, EventArgs e)
        {
            if (CheckBoxValue())
            {
                ...
                try
                {
                    PolicyManageService.UnLockPolicy(PolicyType.Team, this.CurrentUser.UserName, this.txtunlockReason.Text, CurrentCompany.CompanyType == CompanyType.Platform ? OperatorRole.Platform : OperatorRole.User, listIds.ToArray());
                    QueryTeamPolicy(grv_normal.PageIndex + 1);
                }
                catch (Exception ex)
                {
