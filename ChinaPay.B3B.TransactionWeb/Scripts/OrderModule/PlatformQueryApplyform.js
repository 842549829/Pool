function queryApplyform(pageIndex, pageSize) {
    ...
    var targetUrl = "/OrderHandlers/Applyform.ashx/PlatformQueryApplyform";
    ...
    var condition = { "ApplyformId": applyformId, "PNR": pnr, "Passenger": passenger, "AppliedDateRange": { "Lower": startDate, "Upper": endDate }, "ApplyformType": applyType, "RefundStatusText": refundStatus, "PostponeStatusText": posponeStatus, "Purchaser": purchase, "Provider": provider, "Supplier": supplier, "RefundType": refundType, "PurchaseTxt": purchaseTxt, "ProviderTxt": providerTxt, "SupplierTxt": supplierTxt };
    ...
    sendPostRequest(targetUrl, parmater, function (result) {
        ...
        if (result.Applyforms.length == 0) {
            ...
            $.each(result.Applyforms, function (index, item) {
                ...
                applyformContents.push("<td><a href='" + (item.ApplyTypeDesc == "改期" ? "PostponeApplyformDetail.aspx?id=" + item.ApplyformId : item.ApplyTypeDesc == "差错退款" ? "BalanceRefundApplyformDetail.aspx?id=" + item.ApplyformId : "RefundApplyformDetail.aspx?id=" + item.ApplyformId) + "' class='obvious-a'>" + item.ApplyformId + "</a></td>");
                ...
                applyformContents.push("<td>" +
                    (item.NeedProcess ? "<a href='HandleBalanceRefund.aspx?id=" + item.ApplyformId + "' class='ovvious-a'>处理</a><br />" : "")
                +"<a href='" + (item.ApplyTypeDesc == "改期" ?  "PostponeApplyformDetail.aspx?id="  :item.ApplyTypeDesc=="差错退款"?"BalanceRefundApplyformDetail.aspx?id=":"RefundApplyformDetail.aspx?id=" )+ item.ApplyformId + "' class='obvious-a'>" +
                "详情" +"</a><br />" +
                   "<a href='Coordination.aspx?type=1&id=" + item.ApplyformId + "' class='obvious'>" 
                   + "协调</a></br />" + "<a href='../ApplyformLog.aspx?id=" + item.ApplyformId + "' class='obvious-a'>" + "日志</a>" +
                   (item.RenderSupperUnLock ? "<br /><a href='javascript:Unlock(\"" + item.ApplyformId + "\");' id='a" + item.ApplyformId + "' class='obvious-a'>解锁</a>" : "")
                   +
                   (item.EnableQueryPaymentInfo ? "<br /><a href='javascript:QueryPaymentInfo(\"" + item.ApplyformId + "\");' id='b" + item.ApplyformId + "' class='obvious-a'>补单</a>" : "")
                   + "</td>");
                applyformContents.push("</tr>");
            });
