...
function queryApplyform(pageIndex, pageSize) {
    ...
    var targetUrl = "/OrderHandlers/Applyform.ashx/PlatformProcessApplyformNew";
    ...
    if (IsFirstLoad) { //在第一次查询时使用上一次的查询条件加载数据
        var lastData = getCookie("platformProcessApplyform");
        if (lastData != "") {
            parmater = lastData;
        ...
        if (parmater != "") setCookieCurrentPath("platformProcessApplyform", parmater, 1);
    }
    ...
    sendPostRequest(targetUrl, parmater, function (result) {
        ...
        if (result.Applyforms.length == 0) {
            ...
            $.each(result.Applyforms, function (index, item) {
                ...
                applyformContents.push("<td  class='b' " + (isToday ? classStyle : "") + ">" + item.ProcessStatus + "</td>");
                ...
                applyformContents.push("<td><a style='display:" + (item.RequireRevisePrice == true ? "none" : "") + "' href='" + (item.ApplyType == "改期" ? "ProcessPostpone.aspx" : "ProcessRefund.aspx") + "?id=" + item.ApplyformId + "' class='obvious'>" + "处理</a>" +
                                     "<a href='ProcessRevisePrice.aspx?id=" + item.ApplyformId + "&returnUrl=ChangeProcessList.aspx' style='display:" + (item.RequireRevisePrice == true ? "" : "none") + "'>修改价格</a></td>");
                applyformContents.push("</tr>");
            });
