...
function Vaildate_Base() {
    ...
    for (var k = 0; k < $(".parent_div").length; k++) {
        ...
        if ($(".parent_div").eq(k).find(".groupBox2 table tr td[class='B2B']>input[type='checkbox']").is(":checked")) {
            ...
            if ($(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(1).find("input[type='checkbox']:checked").length == 0) {
                bunksFlag = false;
                break;
            }
        ...
        if ($(".parent_div").eq(k).find(".groupBox2 table tr td[class='BSP']>input[type='checkbox']").is(":checked")) {
            ...
            if ($(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(0).find("input[type='checkbox']:checked").length == 0) {
                bunksFlag = false;
                break;
            }
        }
    ...
    if (!flag) {
        alert("第 [ " + groupIndex + " ] 组政策的返佣信息不能为空，且必须是100以内的数字（不包含100）!");
        return false;
    }
    if (!bunksFlag) {
