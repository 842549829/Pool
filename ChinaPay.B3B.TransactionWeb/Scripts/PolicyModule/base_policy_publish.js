...
function GetPolicyValue(actionName) {
    ...
    for (var k = 0; k < $(".parent_div").length; k++) {
        ...
        if ($(".parent_div").eq(k).find(".groupBox2 table tr td[class='BSP']>input[type='checkbox']").is(":checked")) {
            ...
            for (g = 0; g < $(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(0).find("input[type='checkbox']:checked").length; g++) {
                if (g > 0) {
                    bspBerths += ",";
                }
                bspBerths += $(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(0).find("input[type='checkbox']:checked").eq(g).val();
            }
            if ($("input[name='radVoyageType']:checked").val() != "1") {
                policyArrayGroup.push({ "DepartureDateStart": policyDepartureFilghtDataStart, "DepartureDateEnd": policyDepartureFilghtDataEnd, "StartPrintDate": policyStartPrintDate, "DepartureDateFilter": departureDateFilter, "DepartureWeekFilter": departureWeekFilter, "Berths": bspBerths, "TicketType": "BSP", "InternalCommission": (parseFloat(bspInternalCommission)), "SubordinateCommission": (parseFloat(bspSubordinateCommission)), "ProfessionCommission": (parseFloat(bspProfessionCommission)), "ChangePNR": changePnr, "AutoAudit": autoAudit, "SuitReduce": suitReduce, "MultiSuitReduce": multiSuitReduce, "PrintBeforeTwoHours": false });
            } else {
                policyArrayGroup.push({ "DepartureDateStart": policyDepartureFilghtDataStart, "DepartureDateFilter": departureDateFilter, "DepartureWeekFilter": departureWeekFilter, "DepartureDateEnd": policyDepartureFilghtDataEnd, "StartPrintDate": policyStartPrintDate, "Berths": bspBerths, "TicketType": "BSP", "InternalCommission": (parseFloat(bspInternalCommission)), "SubordinateCommission": (parseFloat(bspSubordinateCommission)), "ProfessionCommission": (parseFloat(bspProfessionCommission)), "ChangePNR": changePnr, "AutoAudit": autoAudit, "SuitReduce": suitReduce, "MultiSuitReduce": multiSuitReduce, "PrintBeforeTwoHours": false });
            }
        }
        ...
        if ($(".parent_div").eq(k).find(".groupBox2 table tr td .chkb2b").is(":checked")) {
            ...
            for (g = 0; g < $(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(1).find("input[type='checkbox']:checked").length; g++) {
                if (g > 0) {
                    b2BBerths += ",";
                }
                b2BBerths += $(".parent_div").eq(k).find(".groupBox2 table tr td[class='BunksRad']").eq(1).find("input[type='checkbox']:checked").eq(g).val();
            }

            if ($("input[name='radVoyageType']:checked").val() == "1") {
                policyArrayGroup.push({ "DepartureDateStart": policyDepartureFilghtDataStart, "DepartureDateEnd": policyDepartureFilghtDataEnd, "StartPrintDate": policyStartPrintDate, "DepartureDateFilter": departureDateFilter, "DepartureWeekFilter": departureWeekFilter, "Berths": b2BBerths, "TicketType": "B2B", "InternalCommission": parseFloat(b2BInternalCommission), "SubordinateCommission": parseFloat(b2BSubordinateCommission), "ProfessionCommission": parseFloat(b2BProfessionCommission), "ChangePNR": changePnr, "AutoAudit": autoAudit, "SuitReduce": suitReduce, "MultiSuitReduce": multiSuitReduce, "PrintBeforeTwoHours": printBeforeTwoHours });
            } else {
                policyArrayGroup.push({ "DepartureDateStart": policyDepartureFilghtDataStart, "DepartureDateEnd": policyDepartureFilghtDataEnd, "StartPrintDate": policyStartPrintDate, "DepartureDateFilter": departureDateFilter, "DepartureWeekFilter": departureWeekFilter, "Berths": b2BBerths, "TicketType": "B2B", "InternalCommission": parseFloat(b2BInternalCommission), "SubordinateCommission": parseFloat(b2BSubordinateCommission), "ProfessionCommission": parseFloat(b2BProfessionCommission), "ChangePNR": changePnr, "AutoAudit": autoAudit, "SuitReduce": suitReduce, "MultiSuitReduce": multiSuitReduce, "PrintBeforeTwoHours": printBeforeTwoHours });
            }
        }
...
function ShowOrHides(id) {
    if (id == "dancheng") {
        ...
        $("#liancheng").html("目的地");
        $(".wfjc label").html("适用于往返降舱政策");
        $(".paichutishi").html("提示： 输入排除航线，多条航线之间用“ / ”隔开，（如：单程成都不适用，填写CTU；往返成都到北京不适用，填写CTUPEK）");
        //ClearBunksByAirline();
...
$(function () {
    ...
    $(".btnRefresh").live("click", function () {
        ...
        if (voyage == "dancheng") {
            ...
            voyage = "RoundTrip";
        } else if (voyage == "danchengwanfang") {
            voyage = "OneWay";
            isOneWay = false;
        ...
        if (VaildateRefreshBunksTime(bunksIndex)) {
            //查询舱位
            var param = { "airline": $("#selProvince").val(), "startTime": $(".parent_div").eq(bunksIndex).find("input[type='text']").eq(0).val(), "endTime": $(".parent_div").eq(bunksIndex).find("input[type='text']").eq(1).val(), "startETDZDate": $(".parent_div").eq(bunksIndex).find("input[type='text']").eq(2).val(), "voyage": voyage, "isOneWay": isOneWay };
            sendPostRequest("/PolicyHandlers/PolicyManager.ashx/QueryNormalBunksPolicy", JSON.stringify(param), function (e) {
