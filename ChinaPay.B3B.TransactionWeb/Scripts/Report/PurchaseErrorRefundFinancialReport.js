$(function () {
    ...
    $("#btnDownload").click(function () {
        if (valiate()) {
            ...
            var applierAccount = $.trim($("#ddlOperator").val());
            var purchase = $.trim($("#hfdPurchase").val());

            var amountOfErrorRefundCondition = applyStartDate + ',' + applyEndDate + ',' + orderId + ',' + departure + ',' +
						                                   settleCode + ',' + ticketNo + ',' + applyformId + ',' + arrivals + ',' +
						                                   passenger + ',' + applierAccount + ',' + purchase;
            window.open('ReportDownload.aspx?PurchaseErrorRefundFinancialCondition=' + amountOfErrorRefundCondition, 'newWindow');
        }
    })
})
