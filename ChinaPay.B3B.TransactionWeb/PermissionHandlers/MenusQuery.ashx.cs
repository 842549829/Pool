...
namespace ChinaPay.B3B.TransactionWeb.PermissionHandlers
{
    ...
    public class MenusQuery : BaseHandler
    {
        ...
        public object QueryCompanyMaxValidMenus(string id)
        {
            ...
            var totalPermissions = Service.Permission.Domain.PermissionCollection.Subtract(
                PermissionService.QueryAllValidPermission(Website.Transaction),
                PermissionService.QueryPermissionOfUserRole(UserRole.Platform, Website.Transaction));

            IEnumerable<PermissionView.MenuView> forbiddenPermission = null;
            switch (getUserRole(Guid.Parse(id)))
            {
                case UserRole.Provider:
                    forbiddenPermission = PermissionService.QueryPermissionOfUserRole(UserRole.Provider, Website.Transaction);
                    break;
                case UserRole.Supplier:
                    forbiddenPermission = PermissionService.QueryPermissionOfUserRole(UserRole.Supplier, Website.Transaction);
                    break;
                case UserRole.Purchaser:
                    forbiddenPermission = PermissionService.QueryPermissionOfUserRole(UserRole.Purchaser, Website.Transaction);
                    break;
                default:
                    throw new NotSupportedException();
            }
