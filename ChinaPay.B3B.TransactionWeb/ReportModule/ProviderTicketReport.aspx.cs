...
{
    public partial class ProviderTicketReport : BasePage
    {
        ...
        void pager_CurrentPageChanged(UserControl.Pager sender, int newPage)
        {
            ...
            queryProviderTicket(pagination);
        }

        private void queryProviderTicket(Pagination pagination)
        {
            try
            {
                decimal totaltradeAmount;
                var list = ReportService.QueryProvideTicket(pagination, getCondition(), out totaltradeAmount);
        ...
        private DataTransferObject.Report.ProvideTicketView getCondition()
        {
            ...
            if (!string.IsNullOrWhiteSpace(this.txtTakeOffLower.Text))
            {
                view.TakeoffBeginDate = DateTime.Parse(this.txtTakeOffLower.Text);
            }
            if (!string.IsNullOrWhiteSpace(this.txtTakeOffUpper.Text))
            {
                view.TakeoffEndDate = DateTime.Parse(this.txtTakeOffUpper.Text).AddDays(1).AddMilliseconds(-3);
            }
