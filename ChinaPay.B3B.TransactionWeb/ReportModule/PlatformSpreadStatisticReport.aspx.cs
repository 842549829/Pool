...
{
    public partial class PlatformSpreadStatisticReport : BasePage
    {
        ...
        void Query(Pagination pagination)
        {
            ...
            var list = ReportService.QueryEmployeeSpreadStatisticReport(pagination, DateTime.Parse(txtStartDate.Text), DateTime.Parse(txtEndDate.Text), selCompanyType.Value == "0" ? null : (CompanyType?)int.Parse(selCompanyType.Value), txtEmpolyeeNo.Text.Trim(), out  totalPurchaseAmount, out  totalSupplyAmount, out  totalProvideAmount, out  totalPurchaseCount, out  totalSupplyCount, out  totalProvideCount);
