...
namespace ChinaPay.B3B.TransactionWeb.FlightReserveModule {
    public partial class ChoosePolicy : BasePage {
        public string PatPrice
        {
            get {
                var _PatPrice = GetPATPrice(ReservateSource);
                if (_PatPrice==null)
                {
                    return string.Empty;
                }
                return _PatPrice.Fare.ToString();
            }
        }

        public string PublicFare {
            get {
                var fare = GetPolicyView(ReservateSource);
                if (fare==null)
                {
                    return string.Empty;
                }
                return fare.PublishFare.ToString();
            }
        }
        ...
        private void bindData(string source) {
            ...
            var policy = processPolicyView(source);
        ...
        private PolicyView processPolicyView(string source) {
        ...
        internal static IEnumerable<PassengerView> GetPassengers(string source) {
            if(ReservateSource == source || ImportSource == source) {
                var orderView = GetOrderView(source);
                if (orderView == null)
                {
                    HttpContext.Current.Response.Write("订单已经生成,不能重复生成订单");
                    HttpContext.Current.Response.End();
                    return new List<PassengerView>();
                } //throw new CustomException("订单已经生成,不能重复生成订单");
                return orderView.Passengers;
        ...
        private void bindFlights(IEnumerable<FlightView> flights, string source,PolicyView policy) {
            ...
            IsUsePatPrice.Value = firstFlgiht.BunkType != null & (firstFlgiht.BunkType == BunkType.Economic || firstFlgiht.BunkType == BunkType.FirstOrBusiness) && flights.Count() == 1 ? "1" : "";
        }
