...
// ////////////////////////////////////////////////////////////////////////////////////////////////
// file：Izual.Data.ExecutionBuilder.cs
// description：
// 
...
namespace Izual.Data.Common {
    ...
    public class ExecutionBuilder : DbExpressionVisitor {
        ...
        protected override Expression VisitClientJoin(ClientJoinExpression join) {
            ...
            ConstructorInfo kvpConstructor = typeof(KeyValuePair<,>).MakeGenericType(innerKey.Type, join.Projection.Projector.Type).GetConstructor(new[] {innerKey.Type, join.Projection.Projector.Type});
        ...
        protected override Expression VisitExists(ExistsExpression exists) {
            ...
            QueryType colType = linguist.Language.TypeSystem.GetColumnType(typeof(int));
            SelectExpression newSelect = exists.Select.SetColumns(new[] {new ColumnDeclaration("value", new AggregateExpression(typeof(int), "Count", null, false), colType)});

            var projection = new ProjectionExpression(newSelect, new ColumnExpression(typeof(int), colType, newSelect.Alias, "value"), Aggregator.GetAggregator(typeof(int), typeof(IEnumerable<int>)));
        ...
        protected override Expression VisitOuterJoined(OuterJoinedExpression outer) {
            ...
            if(scope.TryGetValue(column, out reader, out iOrdinal)) {
                return Expression.Condition(Expression.Call(reader, "IsDbNull", null, Expression.Constant(iOrdinal)), Expression.Constant(TypeHelper.GetDefault(outer.Type), outer.Type), expr);
            }
            return expr;
        }
        ...
        #region Nested type: ColumnGatherer
        ...
        #region Nested type: EntityFinder

        private class EntityFinder : DbExpressionVisitor {
            private EntryMapping entity;

            public static EntryMapping Find(Expression expression) {
        ...
        #region Nested type: ProjectionFinder
        ...
        #region Nested type: VariableSubstitutor
