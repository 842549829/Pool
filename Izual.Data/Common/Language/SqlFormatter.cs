...
// ////////////////////////////////////////////////////////////////////////////////////////////////
// file：Izual.Data.SqlFormatter.cs
// description：
// 
...
namespace Izual.Data.Common {
    ...
    public class SqlFormatter : DbExpressionVisitor {
        ...
        protected bool HideColumnAliases {
            get { return hideColumnAliases; }
            set { hideColumnAliases = value; }
        }

        protected bool HideTableAliases {
            get { return hideTableAliases; }
            set { hideTableAliases = value; }
        }
        ...
        protected virtual void WriteColumnName(string columnName) {
            string name = (Language != null) ? Language.Quote(columnName) : columnName;
            Write(name);
        }

        protected virtual void WriteTableName(string tableName) {
            string name = (Language != null) ? Language.Quote(tableName) : tableName;
            Write(name);
        }
        ...
        protected override Expression Visit(Expression exp) {
            ...
            switch(exp.NodeType) {
                ...
                case ExpressionType.Lambda:
                    return base.Visit(((LambdaExpression)exp).Body);
            ...
            if(m.Method.DeclaringType == typeof(Decimal)) {
                switch(m.Method.Name) {
                    ...
                    case "Negate":
                        ...
                        return m;
                    case "Compare":
                        Visit(Expression.Condition(Expression.Equal(m.Arguments[0], m.Arguments[1]), Expression.Constant(0),
                                                   Expression.Condition(Expression.LessThan(m.Arguments[0], m.Arguments[1]), Expression.Constant(-1), Expression.Constant(1))));
                        return m;
                }
            }
                ...
                case ExpressionType.GreaterThanOrEqual:
                    ...
                    if(left.NodeType == ExpressionType.Call && right.NodeType == ExpressionType.Constant) {
                        ...
                        if(ce.Value != null && ce.Value.GetType() == typeof(int) && ((int)ce.Value) == 0) {
                            ...
                            else if((mc.Method.DeclaringType == typeof(string) || mc.Method.DeclaringType == typeof(decimal)) && mc.Method.Name == "Compare" && mc.Method.IsStatic &&
